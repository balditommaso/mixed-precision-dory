...
class DORY_node:
    """
    This class define a generic node in the DORY graph and the allowed operations.
    """
    def __init__(self):
        self.name = None
        self.op_type = None
    ...
    def add_existing_parameter(self, key: str, value: any) -> None:
        if key in self.__dict__:
            self.__dict__[key] = value
        else:
            raise IndexError(
                "Adding {} parameter to a graph node. This parameter does not exist. ".format(key)
            )
            

    def add_existing_dict_parameter(self, dict_parameters: dict) -> None:
    ...
    def check_uninitialized_parameters(self) -> None:
    ...
    def populate_DORY_node(
        ...
        for iterating_input in node_iterating.input:
            is_constant = len(
                [t for t in graph.graph.initializer if t.name == iterating_input]
            )
    ...
    def add_constants(self, graph: onnx.GraphProto) -> None:
        '''
        ...
        for weight_tensor in graph.graph.node:
            if (
                weight_tensor.op_type == 'Constant' 
                and weight_tensor.output[0] in self.input_indexes
            ):
    ...
    def add_special_attributes(self, node_iterating: onnx.NodeProto) -> None:
        '''
        ...
        for attribute in node_iterating.attribute:
            if attribute.name not in ['kernel_shape', 'dilations', 'group', 'strides', 'pads'] or self.name == "Pad":
                if bool(attribute.i):
                    ...
                    raise ValueError(
                        "DORY FRONTEND error. DORY does not find any values for the attribute {}".format(attribute.name)
                    )
    ...
    def export_to_onnx(self) -> dict:
        ...
        for key, value in self.__dict__.items():
            if (
                ...
                and key not in added_parameters 
                and not isinstance(value, bool)
            ):
                node_dict["attribute"].append(
                    {
                        "name": key, 
                        "ints": ([str(value)] if not isinstance(value,list) else [str(v) for v in value])
                    }
                )
